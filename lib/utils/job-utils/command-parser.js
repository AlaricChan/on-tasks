...
function commandParserFactory(Logger, Promise, _) {
    ...
    CommandParser.prototype[lsfileinfo] = function(data){
        if(data.error){
            return Promise.resolve({ source: 'lsfileinfo', error: data.error });
        }
        try {
            var lines = data.stdout.split('\n');
            lines.splice(0, 1);
            ...
            _.forEach(lines, function(line){
                ...
                if(properties[8] === '.' || properties[8] === './' || properties[8] === '..' || properties[8] === '../'){
                    return;
                }
                var item = {};
                item.link_count = properties[1];
                item.owner = properties[2];
                item.group = properties[3];
                item.size = properties[4];
                item.modifed_time = properties[5] + ' ' + properties[6] + ' ' + properties[7];
                item.name = properties[8];
                var fileType = properties[0].substring(0, 1);
                item.permission = {
                    user: properties[0].substring(1, 4),
                    group: properties[0].substring(4, 7),
                    other: properties[0].substring(7, 10)
                };
                switch(fileType){
                    case '-':
                        item.file_type = 'file';
                    ...
                    case 'd':
                        ...
                        break;
                    case 'b':
                        item.file_type = 'block file';
                        break;
                    case 'c':
                        item.file_type = 'character device file';
                        break;
                    case 'p':
                        item.file_type = 'pipe file';
                        break;
                    case 'l':
                        item.file_type = 'link';
                        var nameSplits = properties[8].split(' ');
                        item.name = nameSplits[0];
                        item.link_origin = nameSplits[2];
                        break;
                    case 's':
                        item.file_type = 'socket file';
                        break;
                    default:
                        item.file_type = 'file';
                        break;
            ...
            return Promise.resolve({ data: parsed, source: 'lsfileinfo', store: true });
        }catch(e){
            return Promise.resolve({ source: 'lsfileinfo', error: e });
        }
    };
