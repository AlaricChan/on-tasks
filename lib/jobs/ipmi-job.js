...
function ipmiJobFactory(
    ...
    IpmiJob.prototype._run = function run() {
        ...
        .then(function() {
            ...
            self._subscribeRunIpmiCommand(self.routingKey, 'powerStatus',
                self.createCallback('powerStatus', self.retrievePowerStatus.bind(self)));
        })
        .catch(function(err) {
    ...
    /**
        * Collect node power status
        * @memberOf IpmiJob
        *
        * @param data
        */
    IpmiJob.prototype.retrievePowerStatus = function(data) {
        return ipmitool.retrievePowerStatus(data.host, data.user, data.password)
